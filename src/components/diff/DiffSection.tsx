...
function DiffStatusBadge({ status }: { status: DiffStatus }) {
  ...
  return (
    <span className={`text-2xs font-semibold px-1.5 py-px rounded uppercase tracking-wide ${className}`}>
      {label}
    </span>
  )
}
  ...
  return (
    <div className="mb-6">
      <div className="flex items-center justify-between mb-2">
        ...
        {(diff.status === 'changed' || diff.status === 'added') && diffTargetB && diffTargetB !== 'now' && (
          <button
