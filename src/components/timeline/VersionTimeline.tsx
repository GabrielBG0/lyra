...
export default function VersionTimeline() {
  ...
  return (
    <div
      className="border-t border-border-soft bg-panel flex-shrink-0 transition-all"
      style={{ height: expanded ? 184 : 42 }}
    >
      {/* Header row */}
      <div className="h-[42px] flex items-center px-4 gap-3.5">
        <button
        ...
        >
          ...
          <span className="text-faint text-[10.5px] ml-0.5 px-1.5 py-px bg-elev rounded-full">
            {snapshotHeaders.length} snapshot{snapshotHeaders.length !== 1 ? 's' : ''}
          </span>
          {expanded ? <Icons.ChevronDown size={12} /> : <Icons.ChevronUp size={12} />}
        </button>

        {!expanded && latestSnap && (
          <div className="flex items-center gap-2.5 flex-1 min-w-0 overflow-hidden text-ellipsis whitespace-nowrap text-muted" style={{ fontSize: 11.5 }}>
            <span className="w-[7px] h-[7px] rounded-full bg-accent flex-shrink-0" style={{ boxShadow: '0 0 8px oklch(0.72 0.10 55)' }} />
            <span>Latest: {latestSnap.note ?? 'snapshot'} · {new Date(latestSnap.created_at).toLocaleDateString()}</span>
            <div className="ml-auto flex gap-1 pr-1.5">
              {snapshotHeaders.slice(0, 5).map(s => (
                <span key={s.id} className="w-1 h-1 rounded-full bg-faint opacity-60" />
              ))}
            </div>
          </div>
        )}
        ...
        {!expanded && !latestSnap && (
          <div className="flex-1 text-faint text-xs">No snapshots yet — save a version to track changes.</div>
        )}
      ...
      {expanded && (
        <div className="flex gap-2.5 px-4 pb-3.5 overflow-x-auto">
          ...
          {snapshotHeaders.map((header, i) => (
            <SnapshotCard key={header.id} header={header} index={snapshotHeaders.length - 1 - i} />
          ))}
