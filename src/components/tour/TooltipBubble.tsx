...
export default function TooltipBubble({ step, targetRect }: TooltipBubbleProps) {
  ...
  useEffect(() => {
    ...
    requestAnimationFrame(() => {
      ...
      const { top: rt, bottom: rb, left: rl, right: rr, width: rw, height: rh } = targetRect
  ...
  return (
    ...
    >
      {!step.centered && <TourArrow placement={step.placement} arrowAlign={step.arrowAlign} />}

      <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--color-primary)' }}>
        {step.title}
      </div>
      <div style={{ fontSize: 13, color: 'var(--color-secondary)', lineHeight: 1.6, marginTop: 8 }}>
        {step.body}
      </div>
