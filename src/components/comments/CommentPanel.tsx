...
export default function CommentPanel({ sectionId }: CommentPanelProps) {
  ...
  const handlePost = async () => {
    ...
    const comment = await tauriApi.comment.add(
      filePath,
      sectionId,
      null,
      newText.trim(),
  ...
  return (
    <div className="border-t border-border-soft bg-bg/60 px-4 py-3">
      <div
        className="flex justify-between items-center mb-2.5 text-faint"
        style={{ fontSize: 11 }}
      >
        <span>
      ...
      {loading ? (
        ...
        <>
          ...
          {open.length === 0 && (
            <div className="text-faint text-xs py-2 text-center">
              No open comments
            </div>
          )}
          {showResolved &&
      ...
      <div className="flex items-center gap-2 mt-2.5 px-2.5 py-2 bg-bg border border-border-soft rounded-lg">
        ...
        />
        <label
          className="flex items-center gap-1 text-faint cursor-pointer select-none"
          style={{ fontSize: 10.5 }}
        >
          <input
            type="checkbox"
