...
export default function AppShell({ vaultPath }: AppShellProps) {
  ...
  return (
    <div className="h-full flex flex-col bg-bg text-primary font-ui overflow-hidden">
      <MenuBar onToggleSidebar={() => setSidebarCollapsed(c => !c)} onNewSong={handleNewSong} />

      <div className="flex flex-1 min-h-0">
        {/* Collapsed sidebar strip */}
        {sidebarCollapsed ? (
          <aside className="w-12 flex-shrink-0 bg-panel border-r border-border-soft flex flex-col items-center py-2.5 gap-2">
            <div className="mb-1">
              <LyraLogo size={22} />
            </div>
            <button
