  ...
  updateSection: (id, content) => {
    ...
    const cmd: UpdateSectionCommand = {
      ...
      apply() {
        set((s) => ({
          sections: s.sections.map((sec) =>
            sec.id === id ? { ...sec, content: cmd._after } : sec,
          ),
          isDirty: true,
        }));
        recomputeFind();
      ...
      undo() {
        set((s) => ({
          sections: s.sections.map((sec) =>
            sec.id === id ? { ...sec, content: cmd._before } : sec,
          ),
          isDirty: true,
        }));
        recomputeFind();
    ...
    const cmd: EditorCommand = {
      ...
      apply() {
        set((s) => ({
          ...
          isDirty: true,
          focusedSectionId:
            s.focusedSectionId === id ? null : s.focusedSectionId,
        }));
        recomputeFind();
      },
      undo() {
  ...
  replaceCurrent: (replaceWith) => {
    ...
    const cmd: EditorCommand = {
      ...
      apply() {
        set((s) => ({
          sections: s.sections.map((sec) =>
            sec.id === match.sectionId ? { ...sec, content: newContent } : sec,
          ),
          isDirty: true,
        }));
      },
    ...
    set({
      ...
      findActiveIndex: len > 0 ? Math.min(oldIndex, len - 1) : 0,
    });
  },
  ...
  replaceAll: (replaceWith) => {
    ...
    for (const sid of sectionIds) {
      ...
      for (let i = sectionMatches.length - 1; i >= 0; i--) {
        ...
        content =
          content.slice(0, m.start) + replaceWith + content.slice(m.end);
      }
    ...
    const cmd: EditorCommand = {
      ...
      apply() {
        set((s) => ({
          sections: s.sections.map((sec) =>
            afterMap.has(sec.id)
              ? { ...sec, content: afterMap.get(sec.id)! }
              : sec,
          ),
          isDirty: true,
        }));
      ...
      undo() {
        set((s) => ({
          sections: s.sections.map((sec) =>
            beforeMap.has(sec.id)
              ? { ...sec, content: beforeMap.get(sec.id)! }
              : sec,
          ),
          isDirty: true,
        }));
