...
function makePayload(sections: Section[] = [], overrides: Partial<SongPayload> = {}): SongPayload {
  return {
...
function makeCmd(overrides: Partial<{ applied: number; undone: number }> = {}): EditorCommand & { applied: number; undone: number } {
  const cmd = {
...
describe('updateSection', () => {
  beforeEach(() => {
    useEditorStore.getState().loadSong(makePayload([makeSection('s1', 'original')]))
  })
  ...
  it('does not merge edits to different sections', () => {
    useEditorStore.getState().loadSong(makePayload([makeSection('s1', 'a'), makeSection('s2', 'b')]))
    vi.useFakeTimers()
    useEditorStore.getState().updateSection('s1', 'change 1')
    vi.advanceTimersByTime(500)
...
describe('addSection', () => {
  ...
  it('inserts at specified index', () => {
    useEditorStore.getState().loadSong(makePayload([makeSection('s1'), makeSection('s3')]))
...
describe('removeSection', () => {
  it('removes the section', () => {
    useEditorStore.getState().loadSong(makePayload([makeSection('s1'), makeSection('s2')]))
...
describe('setFindCaseSensitive', () => {
  it('recomputes matches with the new case sensitivity', () => {
    useEditorStore.getState().loadSong(makePayload([makeSection('s1', 'Cat CAT cat')]))
    useEditorStore.getState().setFindQuery('cat', false)
...
describe('setFindWholeWord', () => {
  it('recomputes matches with whole-word constraint', () => {
    useEditorStore.getState().loadSong(makePayload([makeSection('s1', 'cat catfish the cat')]))
    useEditorStore.getState().setFindQuery('cat', false)
    expect(useEditorStore.getState().findMatches).toHaveLength(3)
...
describe('replaceCurrent', () => {
  it('replaces the currently active match', () => {
    useEditorStore.getState().loadSong(makePayload([makeSection('s1', 'the cat sat')]))
    useEditorStore.getState().setFindQuery('cat', false)
  ...
  it('is a no-op when there are no matches', () => {
    useEditorStore.getState().loadSong(makePayload([makeSection('s1', 'hello')]))
    useEditorStore.getState().setFindQuery('zzz', false)
  ...
  it('clamps activeIndex when match count shrinks after replace', () => {
    useEditorStore.getState().loadSong(makePayload([makeSection('s1', 'cat cat')]))
    useEditorStore.getState().setFindQuery('cat', false)
