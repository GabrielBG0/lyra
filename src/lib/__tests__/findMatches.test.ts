...
describe("computeMatches", () => {
  describe("basic matching", () => {
    ...
    it("finds a single match", () => {
      const matches = computeMatches("hello", false, [
        makeSection("s1", "say hello world"),
      ]);
    ...
    it("finds multiple matches in one section", () => {
      const matches = computeMatches("ab", false, [
        makeSection("s1", "ab cd ab ef ab"),
      ]);
    ...
    it("assigns matchIndex within a section", () => {
      const matches = computeMatches("x", false, [
        makeSection("s1", "x y x y x"),
      ]);
  ...
  describe("case sensitivity", () => {
    it("is case-insensitive by default (caseSensitive=false)", () => {
      const matches = computeMatches("Hello", false, [
        makeSection("s1", "hello HELLO Hello"),
      ]);
      expect(matches).toHaveLength(3);
    });

    it("respects caseSensitive=true", () => {
      const matches = computeMatches("Hello", true, [
        makeSection("s1", "hello HELLO Hello"),
      ]);
      expect(matches).toHaveLength(1);
  ...
  describe("whole word matching", () => {
    it("does not match substrings when wholeWord=true", () => {
      const matches = computeMatches(
        "cat",
        false,
        [makeSection("s1", "catfish wildcat concatenate")],
        true,
      );
    ...
    it("matches punctuation-bounded words", () => {
      const matches = computeMatches(
        "love",
        false,
        [makeSection("s1", "I love, you love.")],
        true,
      );
      expect(matches).toHaveLength(2);
    });
  ...
  describe("end offset", () => {
    it("sets end = start + query.length", () => {
      const matches = computeMatches("abcd", false, [
        makeSection("s1", "xxabcdxx"),
      ]);
