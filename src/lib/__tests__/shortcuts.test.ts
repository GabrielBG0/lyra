...
const MOD_N: ShortcutDef = {
  label: "New Song",
  action: "new-song",
  keys: ["mod", "N"],
};
const MOD_SHIFT_S: ShortcutDef = {
  label: "Save Take",
  action: "save-version",
  keys: ["mod", "shift", "S"],
};
const MOD_COMMA: ShortcutDef = {
  label: "Preferences",
  action: "preferences",
  keys: ["mod", ","],
};
...
describe("matchesShortcut", () => {
  describe("mod+key combos", () => {
    it("matches when ctrlKey is pressed", () => {
      expect(
        matchesShortcut(MOD_N, makeEvent({ ctrlKey: true, key: "n" })),
      ).toBe(true);
    });

    it("matches when metaKey is pressed", () => {
      expect(
        matchesShortcut(MOD_N, makeEvent({ metaKey: true, key: "n" })),
      ).toBe(true);
    });
    ...
    it("does not match wrong key", () => {
      expect(
        matchesShortcut(MOD_N, makeEvent({ ctrlKey: true, key: "m" })),
      ).toBe(false);
    });
    ...
    it("does not match when shift is unexpectedly pressed", () => {
      expect(
        matchesShortcut(
          MOD_N,
          makeEvent({ ctrlKey: true, shiftKey: true, key: "n" }),
        ),
      ).toBe(false);
    });
  });
  ...
  describe("mod+shift+key combos", () => {
    it("matches with both mod and shift", () => {
      expect(
        matchesShortcut(
          MOD_SHIFT_S,
          makeEvent({ ctrlKey: true, shiftKey: true, key: "s" }),
        ),
      ).toBe(true);
    });
    ...
    it("does not match without shift", () => {
      expect(
        matchesShortcut(MOD_SHIFT_S, makeEvent({ ctrlKey: true, key: "s" })),
      ).toBe(false);
    });

    it("does not match without mod", () => {
      expect(
        matchesShortcut(MOD_SHIFT_S, makeEvent({ shiftKey: true, key: "s" })),
      ).toBe(false);
    });
  });
  ...
  describe("non-alpha keys", () => {
    it("matches comma key", () => {
      expect(
        matchesShortcut(MOD_COMMA, makeEvent({ ctrlKey: true, key: "," })),
      ).toBe(true);
    });
  ...
  it("no two shortcuts share the same key combo", () => {
    ...
    for (const cat of SHORTCUT_CATEGORIES) {
      for (const s of cat.shortcuts) {
        ...
        expect(
          seen.has(combo),
          `Duplicate shortcut: ${combo} (${s.action})`,
        ).toBe(false);
        seen.add(combo);
      }
    }
