...
export default function App() {
  ...
  useEffect(() => {
    async function init() {
      try {
        ...
        console.log(
          "[Tour] init: tutorial_completed=",
          cfg.tutorial_completed,
          "vault_path=",
          cfg.vault_path,
        );
  ...
  useEffect(() => {
    console.log(
      "[Tour] pendingTour effect: vaultPath=",
      vaultPath,
      "pendingTour=",
      pendingTour,
    );
    ...
    startTour(useSongStore.getState().songs).catch((err) =>
      console.error("[Tour] startTour failed:", err),
    );
  ...
  useEffect(() => {
    const unlisteners = [
      ...
      listen("save-version", () => {
        ...
        if (!filePath) return;
        openSnapshotModal(async (note) => {
          const header = await tauriApi.snapshot.create(
            filePath,
            sections,
            note,
          );
