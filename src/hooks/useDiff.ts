...
export function useDiff() {
  const { filePath, sections, loadedSnapshots, cacheSnapshot, setDiff, clearDiff } =
    useEditorStore()
  ...
  const diffTwoSnapshots = async (snapshotIdA: string, snapshotIdB: string) => {
    ...
    const result = await tauriApi.diff.diffSnapshots(filePath, snapshotIdA, snapshotIdB)
